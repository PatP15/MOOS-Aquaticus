...
if [[ -z $NO_HERON ]]; then
  cd ./surveyor
  # Gus Red
  ./launch_surveyor.sh s r1 r2 $TIME_WARP $LOGDIR -s --start-x=250 --start-y=160 --start-a=240 --role=ATTACK_E > /dev/null &
  sleep 1
  # Luke Red
  ./launch_surveyor.sh t r2 r1 $TIME_WARP $LOGDIR -s --start-x=262 --start-y=156 --start-a=240 --role=DEFEND_E > /dev/null &
  sleep 1
  # Kirk Blue
  ./launch_surveyor.sh u b1 b2 $TIME_WARP $LOGDIR -s --start-x=232 --start-y=75 --start-a=60 --role=ATTACK_MED > /dev/null &
  sleep 1
  # Jing Blue
